...
def safe_upload(func, *args, retries=3):
...
apihelper.READ_TIMEOUT = 600      # 10 min
apihelper.CONNECT_TIMEOUT = 600   # 10 min
apihelper.SESSION_TIME_TO_LIVE = 1800  # 30 min
...
def cb_audio_get(call):
    ...
    try:
        ...
        # ✅ Start processing in a daemon thread with correct args
        threading.Thread(
            target=process_audio,
            args=(call.message, url, title),
            daemon=True
        ).start()
...
def upload_audio_to_channel(file_path, title, chat_id, size_mb):
...
def process_audio(message, url, title):
    chat_id = message.chat.id
    try:
        ...
        if size_mb <= 50:
            # Directly send small files
            with open(file_path, "rb") as af:
                bot.send_audio(chat_id, af, caption=f"🎵 {title}")
            os.remove(file_path)
        else:
            bot.send_message(chat_id, "⏳ Uploading to channel, please wait...")

            # Threaded upload for large files
            def upload_and_cleanup():
                try:
                    upload_audio_to_channel(file_path, title, chat_id, size_mb)
                finally:
            ...
            threading.Thread(
    target=safe_upload,
    args=(upload_to_channel, file_path, title, 0, chat_id, size_mb),  # duration=0 for audio
    daemon=True
).start()
    ...
    try:
        ...
        try:
            if size_mb <= 50:
                ...
                bot.send_message(chat_id, "⏳ Uploading to channel, please wait...")
                threading.Thread(
                    target=upload_to_channel,
                    args=(final_path, title, duration, chat_id, size_mb),
                    daemon=True
                ).start()  # ✅ no join
        except Exception as e:
            bot.send_message(chat_id, f"❌ Sending failed: {e}")
